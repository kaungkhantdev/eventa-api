...
export const envSchema = z.object({
  ...
  // Object storage (US-DISC-11). Uploads go straight from the browser to the
  // bucket via a presigned URL — the bytes never pass through the API.
  //
  // One backend, always S3. Locally that bucket is MinIO, which speaks the
  // same API and is reached through S3_ENDPOINT below: a different address,
  // not a different implementation. The bucket and region are therefore
  // required, exactly like DATABASE_URL — an upload has nowhere to go
  // without them, and boot is the moment to say so.
